        ...
        public Models.ResponseModels.OrderSearchResponse GetOrdersSearchResponse(Models.RequestModels.OrderSearchRequest searchRequest)
        {
            ...
            Expression<Func<Order, bool>> query =
                    s => (
                            ...
                            && (searchRequest.OrderFromDate == null || 
                            DbFunctions.TruncateTime(s.RecCreatedDate) >= DbFunctions.TruncateTime(searchRequest.OrderFromDate.Value))
                            && (searchRequest.OrderToDate == null ||
                            DbFunctions.TruncateTime(s.RecCreatedDate) <= DbFunctions.TruncateTime(searchRequest.OrderToDate.Value))
                            && (s.IsDeleted!=true)
                            )
                        );
